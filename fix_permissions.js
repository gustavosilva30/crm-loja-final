...
async function run() {
    const { data, error } = await supabase.from('atendentes').update({ perm_config: true }).ilike('email', '%pecasdourados_paccote%')
    if (error) {
        console.error(error)
    } else {
        console.log("Updated permissions for pecarsdourados_paccote@hotmail.com")
    }
}
